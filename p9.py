...
def agregar_numero():
    """Agrega un número y actualiza la suma parcial"""
    global lista_numeros, suma_parcial, proceso_finalizado
    ...
    if suma_parcial > LIMITE:
        finalizar_proceso()
    else:
        # Limpiar campo para siguiente número
        entrada_numero.value = ''
        entrada_numero.focus()
        ui.notify(f'➕ Número {numero} agregado. Suma parcial: {suma_parcial}', type='info')
...
def actualizar_interfaz():
    ...
    lbl_suma.set_text(f'💰 Suma parcial: {suma_parcial}')

def finalizar_proceso():
    """Finaliza el proceso cuando la suma supera 100"""
    ...
    lbl_lista.set_text(f'📋 Lista de números: {lista_numeros}')
    lbl_cantidad.set_text(f'🔢 Cantidad de números: {len(lista_numeros)}')
    lbl_suma.set_text(f'💰 Suma final: {suma_parcial}')
    
    lbl_estado.set_text(f'✅ Proceso finalizado (Suma {suma_parcial} > {LIMITE})')
    ...
    ui.notify(f'🏁 Límite superado! Suma final: {suma_parcial}', type='positive')
...
def reiniciar():
    ...
    lbl_suma.set_text('💰 Suma parcial: 0')
    lbl_estado.set_text('⏳ Estado: Ingrese números hasta superar 100')
...
def crear_interfaz():
    ...
    with ui.card().classes('w-full max-w-2xl mx-auto p-6 shadow-lg'):
        ui.label('🎯 Suma de Números hasta Superar Límite').classes('text-2xl font-bold mb-2 text-center')
        ui.label('Ingrese números enteros. La suma debe superar 100.').classes('text-gray-600 mb-4 text-center')
    
        # Entrada de números
        ui.label('📝 Ingrese un número:').classes('mb-2 font-semibold')
        entrada_numero = ui.input(label='Número', placeholder='Ej: 25').classes('w-full')
        ...
        # Estado del proceso
        lbl_estado = ui.label('⏳ Estado: Ingrese números hasta superar 100').classes('text-lg font-semibold mb-4')
        ...
        with ui.card().classes('w-full p-4 bg-blue-50'):
            ...
            lbl_suma = ui.label('💰 Suma parcial: 0').classes('text-xl font-bold text-green-600 my-2')
    
        ui.label(f'💡 Tip: La suma debe ser mayor que {LIMITE} para finalizar.').classes('text-sm text-gray-500 text-center mt-2')
